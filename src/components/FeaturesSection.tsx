        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {features.map((f, i) => (
            <a
              href="#download"
              key={f.title}
              className={`fade-up fade-up-d${(i % 3) + 1} group block bg-surface border border-border rounded-2xl p-9 transition-all duration-200 relative overflow-hidden no-underline text-foreground hover:border-primary/40 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)]`}
            >
              ...
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl mb-6 ${f.colorClass}`}>
                {f.icon}
              </div>
              <h3 className="font-heading text-lg font-bold mb-3 tracking-tight">{f.title}</h3>
              <p className="text-muted-foreground text-sm font-light leading-relaxed">{f.desc}</p>
              <div className="absolute bottom-6 right-6 text-dim text-base opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all text-electric">↗</div>
            </a>
          ))}
        </div>
      </div>
